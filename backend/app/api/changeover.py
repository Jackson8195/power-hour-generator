...
async def build_video_changeover(
    ...
    if not row:
        raise HTTPException(status_code=404, detail="No changeover clip for this project")
    if row.source_type not in ("youtube", "local_video"):
        raise HTTPException(status_code=422, detail="Build endpoint is only for video-mode changeover clips")
    if not row.raw_video_path or not Path(row.raw_video_path).exists():
        raise HTTPException(status_code=422, detail="Raw video file not found — re-download or re-upload")
